...
export class Hangman {
  readonly phrase: string; // normalised: UPPERCASE, single spaces
  ...
  constructor(phrase: string) {
    const normalized = phrase.toUpperCase().split(/\s+/).filter(Boolean).join(" ");
    if (!normalized) throw new Error("phrase must contain at least one letter");
    for (const ch of normalized) {
      if (ch !== " " && !isAZ(ch)) {
        throw new Error(`phrase may contain only A-Z and spaces; got ${JSON.stringify(ch)}`);
      }
    }
    this.phrase = normalized;
    this.words = normalized.split(" ");
    this.letters = new Set([...normalized].filter((c) => c !== " "));
  }
  ...
  guess(raw: string): GuessResult {
    ...
    const s = raw.toUpperCase().split(/\s+/).filter(Boolean).join(" ");
    if (!s) return "invalid";
    if (s.length === 1) return this.guessLetter(s);
    if (![...s].every((c) => c === " " || isAZ(c))) return "invalid";
  ...
  private guessWord(guess: string): GuessResult {
    ...
    // phrase. A spaceless guess matches a single word. The design reveals by
    // letter, so a correct word/phrase reveals its letters (everywhere). A
    // wrong word/phrase guess costs nothing — misses are for letter guesses.
    if (guess.includes(" ")) {
      if (guess === this.phrase) {
        this.pushHistory();
    ...
    if (this.words.includes(guess)) {
      const fresh = [...guess].filter((c) => !this.revealed.has(c));
      if (fresh.length > 0) {
